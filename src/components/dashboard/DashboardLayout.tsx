...
const DashboardLayout = ({ activeTab, onTabChange, children }: DashboardLayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-30 bg-background pt-14 overflow-y-auto safe-bottom">
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-background border-t border-border flex safe-bottom">
        {mobileNav.map((item) => (
          <button
            key={item.id}
            onClick={() => onTabChange(item.id)}
            className={`flex-1 flex flex-col items-center gap-1 py-2.5 text-[11px] font-heading transition-colors ${
